...
interface BusinessCardProps extends BusinessEntity {}
...
export function BusinessCard(props: BusinessCardProps) {
  ...
  function addEventListeners(): void {
    ...
    businessImg?.addEventListener('error', function setDefaultImage() {
      this.src = IMAGE_NOT_FOUND_PATH;
    });

    const aTag = componentContainer.querySelector('a');

    aTag?.addEventListener('click', function (event: Event) {
